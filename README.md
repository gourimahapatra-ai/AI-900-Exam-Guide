...
flowchart TD
    ...
    A --> V[Vision]
    V --> V1[Image Analysis]
    V1 --> V1a[Object Detection]
    V1 --> V1b[Scene Understanding]
    V1 --> V1c[Image Tagging]
    V1 --> V1d[Thumbnail Generation]
    V --> V2[Face]
    V2 --> V2a[Face Detection]
    V2 --> V2b[Face Verification]
    V2 --> V2c[Face Grouping]
    V --> V3[OCR (Read)]
    V3 --> V3a[Printed Text Extraction]
    V3 --> V3b[Handwritten Text]
    V3 --> V3c[Multi-language Support]

    %% Speech
    A --> S[Speech]
    S --> S1[Speech to Text]
    S1 --> S1a[Real-time Transcription]
    S1 --> S1b[Batch Transcription]
    S --> S2[Text to Speech]
    S2 --> S2a[Neural Voices]
    S2 --> S2b[Voice Styles]
    S2 --> S2c[SSML Support]
    S --> S3[Speech Translation]
    S3 --> S3a[Real-time Translation]
    S3 --> S3b[Multi-language Output]

    %% Language
    A --> L[Language]
    L --> L1[Text Analytics]
    L1 --> L1a[Named Entity Recognition]
    L1 --> L1b[PII Detection]
    L1 --> L1c[Entity Linking]
    L --> L2[Sentiment Analysis]
    L2 --> L2a[Opinion Mining]
    L2 --> L2b[Emotion Detection]
    L --> L3[Key Phrase Extraction]
    L3 --> L3a[Automatic Topic Identification]
    L --> L4[Language Detection]
    L4 --> L4a[Auto Language Identification]
    L --> L5[Question Answering]
    L5 --> L5a[FAQ Knowledge Bases]
    L5 --> L5b[Conversational Answers]

    %% Decision
    A --> D[Decision]
    D --> D1[Anomaly Detector]
    D1 --> D1a[Time-series Analysis]
    D1 --> D1b[Pattern Deviation Detection]
    D --> D2[Content Safety]
    D2 --> D2a[Hate & Abuse Detection]
    D2 --> D2b[Adult Content Filtering]
    D2 --> D2c[Violence Detection]

    %% Document Intelligence
    A --> DI[Document Intelligence]
    DI --> DI1[Form Recognizer]
    DI1 --> DI1a[Structured Data Extraction]
    DI1 --> DI1b[Custom Form Models]
    DI1 --> DI1c[Prebuilt Models]
    DI --> DI2[Invoice/Receipt Extraction]
    DI2 --> DI2a[Vendor Name]
    DI2 --> DI2b[Amounts & Tax]
    DI2 --> DI2c[Date & Line Items]
